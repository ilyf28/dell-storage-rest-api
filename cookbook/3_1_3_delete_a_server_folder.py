...
payload = {}
REST = '/StorageCenter/ScServerFolder/%s' % srvFolderList['RestTest_001']['instanceId']
completeURL = '%s%s' % (baseURL, REST if REST[0] != '/' else REST[1:])
json_data = connection.delete(completeURL, headers=header, verify=verify_cert)
stdout = json.loads(json_data.text)
print stdout
del srvFolderList['RestTest_001']
